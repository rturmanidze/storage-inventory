...
# heads` fails with "overlaps" because both rows are treated as current heads and
# their upgrade paths share common revisions.  The fix is to keep only the
# lexicographically largest (most-recent) revision and let Alembic continue from there.
python3 - <<'PYEOF'
import os, sys
try:
